...
int bma222_set_range(unsigned char Range)
{
   ...
   unsigned char data1;
      ...
      if (Range < C_Four_U8X)
      {
         comres = yas_acc_i2c_read(C_Zero_U8X, BMA222_RANGE_SEL_REG, &data1, C_One_U8X );
         switch (Range)
         {
            case C_Zero_U8X:
         ...
         comres += yas_acc_i2c_write(C_Zero_U8X, BMA222_RANGE_SEL_REG, &data1, C_One_U8X);
      }
      else
      {
...
unsigned char bma222_set_mode(unsigned char Mode)
{
   ...
   unsigned char data1;
      if (Mode < C_Three_U8X)
      {
         comres = yas_acc_i2c_read(C_Zero_U8X , BMA222_EN_LOW_POWER__REG, &data1, C_One_U8X );
         switch (Mode)
         {
         ...
         comres += yas_acc_i2c_write(C_Zero_U8X, BMA222_RANGE_SEL_REG, &data1, C_One_U8X);
         comres += yas_acc_i2c_write(C_Zero_U8X, BMA222_EN_LOW_POWER__REG, &data1, C_One_U8X);
      }
      else
      {
         comres = E_OUT_OF_RANGE ;
...
int bma222_set_bandwidth(unsigned char BW)
{
   ...
   int Bandwidth ;
      if (BW < C_Eight_U8X)
      {
         ...
         comres = yas_acc_i2c_read(C_Zero_U8X, BMA222_BANDWIDTH__REG, &data, C_One_U8X );
         data = BMA222_SET_BITSLICE(data, BMA222_BANDWIDTH, Bandwidth );
         comres += yas_acc_i2c_write(C_Zero_U8X, BMA222_BANDWIDTH__REG, &data, C_One_U8X );
      }
      else
      {
         comres = E_OUT_OF_RANGE ;
...
static int yas_acc_i2c_write(uint8_t slave, uint8_t reg, const uint8_t *buf, int len)
{
    ...
    struct yas_acc_private_data *data = yas_acc_get_data();
     //printk(KERN_ERR "yas acc i2c write +\n");
...
static int yas_acc_i2c_read(uint8_t slave, uint8_t reg, uint8_t *buf, int len)
{
    ...
    struct yas_acc_private_data *data = yas_acc_get_data();
     //printk(KERN_ERR "yas acc i2c read +\n");
...
static ssize_t bma222_fs_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    //printk(KERN_ERR "yas acc value x: %d,y: %d,z: %d\n", acc.x, acc.y, acc.z);
...
int bma222_read_accel_xyz(bma222acc_t * acc)
{
   ...
   unsigned char data[6];
      ...
      {
         case 1:
            comres = yas_acc_i2c_read(C_Zero_U8X, BMA222_ACC_X10_LSB__REG, data, 6);
            ...
            break;
         case 2:
            comres = yas_acc_i2c_read(C_Zero_U8X, BMA222_ACC_X8_LSB__REG, data, 6);
...
#endif
//printk(KERN_ERR "bma222_init_ end \n");
...
static int yas_acc_set_enable(struct yas_acc_driver *driver, int enable)
{
	...
	//printk(KERN_ERR "bma222_init_ enable %d, %s %d %d %d\n",__LINE__,__func__,enable,delay,actual_delay(delay));
	if (yas_acc_ischg_enable(driver, enable)) {
		if (enable) {
#ifdef BMA222_SENSOR_DEBUG
			printk(KERN_ERR "bma222_init_ enable change start %d, %s \n",__LINE__,__func__);
#endif
			driver->set_enable(enable);
			schedule_delayed_work(&data->work, delay_to_jiffies(delay) + 1);
		} else {
#ifdef BMA222_SENSOR_DEBUG
			printk(KERN_ERR "bma222_init_ disable change cancel %d, %s \n",__LINE__,__func__);
#endif
...
static int yas_acc_set_delay(struct yas_acc_driver *driver, int delay)
{
    ...
    if (driver->get_enable()) {
        ...
        schedule_delayed_work(&data->work, delay_to_jiffies(delay) + 1);
	//printk(KERN_ERR "bma222_init_ enable %d, %s %d  %d\n",__LINE__,__func__,delay,actual_delay(delay));
    } else {
#ifdef BMA222_SENSOR_DEBUG
			printk(KERN_ERR "bma222_init_ disable %d, %s %d  %d\n",__LINE__,__func__,delay,actual_delay(delay));
#endif
        driver->set_delay(actual_delay(delay));
    }
...
static int yas_acc_input_init(struct yas_acc_private_data *data)
{
    ...
    if (err < 0) {
		...
		printk(KERN_ERR "bma222_init_ error %d, %s \n",__LINE__,__func__);
        return err;
    }
    data->input = dev;
...
#ifdef BMA222_SENSOR_DEBUG
	    printk(KERN_ERR "bma222_init_ enable cancel %d, %s \n",__LINE__,__func__);
#endif
            cancel_delayed_work_sync(&data->work);
...
static int yas_acc_suspend(OMAP_GPIO_I2C_CLIENT *client, pm_message_t mesg)
{
    ...
    if (data->suspend == 0) {
        ...
        if (data->suspend_enable) {
#ifdef BMA222_SENSOR_DEBUG
	    printk(KERN_ERR "bma222_init_ enable suspend cancel %d, %s \n",__LINE__,__func__);
#endif
            cancel_delayed_work_sync(&data->work);
            yas_acc_set_enable(driver, 0);
        }
...
static int yas_acc_resume(OMAP_GPIO_I2C_CLIENT *client)
{
    ...
    if (data->suspend == 1) {
        if (data->suspend_enable) {
#ifdef BMA222_SENSOR_DEBUG
	    printk(KERN_ERR "bma222_init_ enable resume %d, %s \n",__LINE__,__func__);
#endif
            delay = yas_acc_get_delay(driver);
            schedule_delayed_work(&data->work, delay_to_jiffies(delay) + 1);
            yas_acc_set_enable(driver, 1);
...
//phill-it 0808
static DEVICE_ATTR(calibrate, S_IRUGO|S_IWUSR|S_IWGRP,bma222_calibration_show, bma222_calibration_store);
static DEVICE_ATTR(acc_file, S_IRUGO|S_IWUSR|S_IWGRP, bma222_fs_read, bma222_fs_write);
static struct device_attribute *acc_class_attributes[] = {
	&dev_attr_calibrate,
	&dev_attr_acc_file,
};
...
//dev/accelerometer
	result = register_chrdev( BMA222_MAJOR, YAS_ACC_KERNEL_NAME, &acc_fops);
	if (result < 0) 
	{
		return result;
	}
	...
	if (IS_ERR(acc_class)) 
	{
		unregister_chrdev( BMA222_MAJOR, YAS_ACC_KERNEL_NAME);
		printk("Failed to class create\n");
	...
	for (i = 0; i < ARRAY_SIZE(acc_class_attributes); ++i) {
			if (device_create_file(dev_t,acc_class_attributes[i]) < 0)
					printk("Failed to create accelerometer device file!\n");
	}
	if (IS_ERR(dev_t)) 
	{
	...
	printk(KERN_ERR "bma222_init_proc end-\n");
