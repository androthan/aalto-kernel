...
enum {
	bma222_cal_negative_1 = -1,
	bma222_cal_standard   = 0,
	bma222_cal_positive_1 = 1,
};
...
static int yas_bma222_measure(int *out_data, int *out_raw)
{
    ...
    //printk("bma222 raw data  X:%d Y:%d Z:%d \n", raw[0], raw[1], raw[2]);
    for(i =0; i < 2; i++){
	    switch(raw[i]){
		    /*
		    case bma222_cal_positive_1: 
			    raw[i] = (raw[i] >> 1);
			    break;*/
		    case bma222_cal_negative_1:
			    raw[i] = bma222_cal_standard; 
			    break;
	    }
    }
