...
static int omap2_dma_handle_ch(int ch)
{
	...
	if (unlikely(status & OMAP2_DMA_TRANS_ERR_IRQ)) {
		 ...
		 */
		 /*
		 *Temporary modification (skip the deativation) for exceptional cases. sejong
		 */
		//if (cpu_class_is_omap2()) {
			...
			/* Enable the channel manually */
			//u32 ccr_enable;
			//ccr_enable = dma_read(CCR(ch));
			//ccr_enable |= OMAP_DMA_CCR_EN;
			//dma_write(ccr_enable, CCR(ch));
		//}
