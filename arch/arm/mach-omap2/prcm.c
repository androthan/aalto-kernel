				...
				const char *name)
{
	...
	else
	{
	    ...
	    if((strcmp(name,"dss1_alwon_fck")==0) )
	    	{

	    	    	printk(" IDLEST_DSS =  %x  FCLKEN_DSS = %x ICLKEN_DSS = %x  \n",  __raw_readl(reg), omap_readl(0x48004E00) ,omap_readl(0x48004E10) );

			if(omap_readl(0x48004E00) & 0x01)
				printk(" FCLKEN_DSS is already enabled \n");
			else
				omap_writel(omap_readl(0x48004E00) |0x01, 0x48004E00); //CM_FCLKEN_DSS
				
                    if(omap_readl(0x48004E10) & 0x01)
				printk(" ICLKEN_DSS is already enabled \n");
		      else
			{
				omap_writel(0x00, 0x48004E30);//CM_AUTOIDLE_DSS disable
			       omap_writel(omap_readl(0x48004E10) |0x01, 0x48004E10); //CM_ICLKEN_DSS
				omap_writel(0x01, 0x48004E30); //CM_AUTOIDLE_DSS enable
			}
			printk("After  IDLEST_DSS = %x FCLKEN_DSS = %x ICLKEN_DSS = %x    \n" ,__raw_readl(reg), omap_readl(0x48004E00) ,omap_readl(0x48004E10));
