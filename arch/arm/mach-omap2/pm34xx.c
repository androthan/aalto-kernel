...
static void omap3_enable_io_chain(void)
{
	...
	/*
         ...
         */
		prm_set_mod_reg_bits(OMAP3430_ST_IO_CHAIN_MASK, WKUP_MOD, PM_WKEN);
	}
}
...
int omap3_bypass_cmd(u8 slave_addr, u8 reg_addr, u8 cmd) {
	...
	vc_bypass_value = prm_set_mod_reg_bits(OMAP3430_VALID_MASK, OMAP3430_GR_MOD,
					OMAP3_PRM_VC_BYPASS_VAL_OFFSET);

	while ((vc_bypass_value & OMAP3430_VALID_MASK) != 0x0) {
		loop_cnt++;
		if (retries_cnt > 10) {
...
//idle current optimisation 	
	...
	if (omap_rev() == OMAP3630_REV_ES1_1){	
		//printk(" OMAP3630_REV_ES1_1 \n");
        /* Work Around 0 for CKE1 Bug Starts
         * Manually switch VDD2 to 1.087V, here we use an I2C command to the power IC
         */
         if(core_next_state==PWRDM_POWER_OFF)
		 	omap3_bypass_cmd(0x12,0x01,0x27); //using bypass command to set vdd2 voltage to 1.08v
        /* Work Around 0 for CKE1 Bug Ends */
	}
...
//	omap3_intc_prepare_idle();
	...
	*/
	...
	if (omap_rev() == OMAP3630_REV_ES1_1){	
	               /* Work Around 1 for CKE1 Bug Starts
                 * Disable the Self-Refresh on auto count feature
                 * before transitioning to OFF mode: set SDRC_POWER_REG[5:4]: CLKCTRL=0x0
                 */
                        u32 sdrc_regval = 0;
                        sdrc_regval = sdrc_read_reg(SDRC_POWER);
                        sdrc_regval &= ~(0x30);
                        sdrc_write_reg(sdrc_regval, SDRC_POWER);
                /* Work Around 1 for CKE1 Bug Ends */

                /* Work Around 2 for CKE1 Bug Starts
                 * sw overrides the value saved in scratchpad memory
                 * by writing MUXMODE =0x001f to @0x48002834 before transitioning to OFF
                 */
                omap_writel(0x001f, 0x48002834);
                /* Work Around 2 for CKE1 Bug Ends */
	}
	...
	if (omap_rev() == OMAP3630_REV_ES1_1){	
	        /* Work Around 3 for CKE1 Bug Starts
         * SW reconfigures CKE1 mux mode to mode 0 by using
         * following sequence that guaranties no glitch
         */
        omap_ctrl_writel(0x001b, 0x264);
        omap_ctrl_writel(0x0019, 0x264);
        omap_ctrl_writel(0x0018, 0x264);

        /* Missing auto refresh on CS1 between CKE1 rising edge to ROM code MR access needs to be compensated.
         * 400 manual auto refresh are required: write 400x SDRC_MANUAL_1[3:0]:CMDCODE=0x2
         */
        static u32 counter;

        for(counter = 0; counter < 400; counter++){
			u32 sdrc_regval = 0;
			sdrc_regval = sdrc_read_reg(SDRC_MANUAL_1);
			sdrc_regval =0x2; 
			sdrc_write_reg(sdrc_regval, SDRC_MANUAL_1);
        }
        /* Work Around 3 for CKE1 Bug Ends */
	}
