...
//extern void wm1811_ear_micbias_on_off(struct snd_soc_codec *codec, u16 on);
...
EXPORRT_SYMBOL_GPL(earjack_min_max_adc);
...
static void ear_adc_caculrator(struct work_struct *work)
{
	...
	//wm1811_ear_micbias_on_off(wm1811_get_snd_soc_codec(), ON);
	wm1811_ear_micbias_set(wm1811_get_snd_soc_codec(), ON);
	...
	if (state)
	{
		...
		else 
		{
			...
			//wm1811_ear_micbias_on_off(wm1811_get_snd_soc_codec(), OFF);
			wm1811_ear_micbias_set(wm1811_get_snd_soc_codec(), OFF);
	...
	else
	{
		...
		WM1811_Switch_To_Earphone(wm1811_get_snd_soc_codec(), OFF);		
