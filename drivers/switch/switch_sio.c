...
extern bool is_jig_uart_on(void);
...
static ssize_t keystring_sel_show(struct device *dev, struct device_attribute *attr, char *buf)
{	
	char tmp = 'D';

	if (is_jig_uart_on() == true)
	{
		tmp = 'E';
	}
...
static DEVICE_ATTR(keyString_sel, 0664, keystring_sel_show, NULL);
/*sysfs for HOME dock state */
static DEVICE_ATTR(dock, 0664, dock_switch_show, dock_switch_store);
