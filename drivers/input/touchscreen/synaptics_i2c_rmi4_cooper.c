...
#endif
// taejin. merge touch driver temporary
#if 0
...
#else
	//gpio_set_value( OMAP_GPIO_TSP_SCL , 0 ); 
	//gpio_set_value( OMAP_GPIO_TSP_SDA , 0 ); 
	gpio_set_value( OMAP_GPIO_TOUCH_INT , 0 ); 

	msleep( 5 );

//	gpio_set_value( OMAP_GPIO_TSP_SCL , 1 ); 
//	gpio_set_value( OMAP_GPIO_TSP_SDA , 1 ); 
...
*/
	...
	for ( i= 0; i<MAX_USING_FINGER_NUM; i++ )
	{
		...
		//			if(fingerInfo[1].id ==0)
					{
						...
						input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, (((fingerInfo[2].id)<<8)|fingerInfo[2].z));
		...
		input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, (((fingerInfo[i].id)<<8)|fingerInfo[i].z));
		input_mt_sync(ts->input_dev);
...
//#else
	vmmc2 = regulator_get( NULL, "vmmc2" );
	if( IS_ERR( vmmc2 ) )
		printk( "Fail to register vmmc2 in probe1 using regulator framework!\n" );
...
#if 1
	twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, LDO_3_0V, TWL4030_VMMC2_DEDICATED);
	twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, (DEV_GRP_P1 << 5), TWL4030_VMMC2_DEV_GRP);
#endif
...
//#else
	vmmc2 = regulator_get( NULL, "vmmc2" );                         
	if( IS_ERR( vmmc2 ) )                                                   
		printk( "Fail to register vmmc2 using regulator framework!\n" );
#endif
