...
#include "zinitix_touch_firmware_usa.h"
#else
	...
	if (get_hw_revision() <= 0x07) {
		if(vreg_touch1 == NULL)	{
			...
			if(IS_ERR(vreg_touch1))
				printk("[TSP] Fail to register vreg_touch1(vmmc2) in touch driver\n");
		}
		if(vreg_touch2 == NULL) {
			vreg_touch2 = regulator_get(NULL, "vdda_dac1");
			if(IS_ERR(vreg_touch2))
				printk("[TSP] Fail to register vreg_touch2(vdac2) in touch driver\n");
		}

		if(ctl == POWER_OFF) //power off
		{
			printk("[TSP] tsp_power off\n");
		...
		if (ctl == POWER_OFF) { //power off
			...
			if (ret33 < 0) {
				printk("[TSP] fail to get gpio : %d, res : %d\n", OMAP_GPIO_AMP_SHDN, ret33);
			}
...
static void zinitix_touch_tmr_work(struct work_struct *work)
{
	...
	printk("[TSP] error. timeout occured. maybe ts device dead. so reset & reinit.\r\n");
	...
	return;
fail_time_out_init:
	printk("[TSP] tmr work : restart error\r\n");	
	ts_esd_timer_start(ZINITIX_CHECK_ESD_TIMER, touch_dev);
	touch_dev->work_proceedure = TS_NO_WORK;
	enable_irq(touch_dev->irq);
	up(&touch_dev->work_proceedure_lock);
...
{
	u16 newVersion = (u16) (m_firmware_data[0] | (m_firmware_data[1] << 8));

	printk(KERN_INFO "[TSP] cur Version = 0x%.4x, new Version = 0x%.4x\n", curVersion, newVersion);
...
u8 ts_upgrade_firmware(zinitix_touch_dev* touch_dev, const u8 *firmware_data, u32 size)
{
	...
	printk("[TSP] TSP frimware show : %02d\n", chip_firmware_version);
	...
	do{
...
#else
		printk("[TSP] Erase Flash w/o HW_Calibration\n");
		...
		printk("[TSP] writing firmware data\n");			
		...
		for(flash_addr= 0; flash_addr< size; )
		{
			...
			if(flash_addr >= CALIBRATION_AREA) {
				printk("[TSP] TSP, pass calibration data area\n");
				break;
			}
#endif
		}

		printk("[TSP] read firmware data\n");					
		for(flash_addr= 0; flash_addr< size; )
		{
...
#endif
		// verify
		printk("[TSP] verify firmware data\n");
		if(memcmp((u8*)&firmware_data[0], (u8*)&verify_data[0], size) == 0)
		{
#if USE_HW_CALIBRATION
			printk("[TSP] TSP, need_hw_calibration is 1\n");
			need_hw_calibration = 1;
#endif
			printk("[TSP] upgrade finished\n");
			kfree(verify_data);
			kfree(temp_data);
			ts_power_control(touch_dev, RESET_LOW); //reset pin low
			ts_power_control(touch_dev, POWER_OFF); //power off
		...
		printk("[TSP] upgrade fail : so retry... (%d)\n", ++retry_cnt);
	}while(1);
...
bool ts_init_touch(zinitix_touch_dev* touch_dev)
{
	...
	if(touch_dev == NULL)
	{
		printk("[TSP] error touch_dev == null?\r\n");
		goto fail_init;
	}
...
retry_init:
	...
	{
		printk("[TSP] fail to write interrupt register\r\n");
		goto fail_init;
	}
	...
	{
		printk("[TSP] fail to read chip revision\r\n");
		goto fail_init;
	}	
	printk("[TSP] zinitix touch chip revision id = %x\r\n", chip_revision);
	...
	{
		printk("[TSP] start upgrade firmware\n");
		...
		printk("[TSP] zinitix touch chip renewed firmware version = %x\r\n", chip_firmware_version);
		...
		for(i=0; i < MAX_REG_COUNT; i++)
		{
			if(m_reg_data[i].valid == 1)
			{
				...
				if(memcmp((char*)&m_reg_data[i].reg_val, (char*)&stmp, 2)!=0)		//if(m_reg_data[i].reg_val != stmp)
					printk("[TSP] register data is different. (addr = 0x%02X , %d != %d)\r\n", i, m_reg_data[i].reg_val, stmp);					
			}
		}
	...
	if (need_hw_calibration) {
		printk("[TSP] TSP, need calibration\n");
		if(zinitix_bit_test(chip_eeprom_info, 0))		// hw calibration bit
		{
			printk("[TSP] TSP, do calibration\n");
			 // h/w calibration
			udelay(100);
			for(i=0;i<10;i++)
			{
...
#endif
{
	...
	if(touch_dev->raw_mode_flag == TOUCH_TEST_RAW_MODE)	// test mode
	{
		if (ts_write_reg(touch_dev->client, ZINITIX_PERIODICAL_INTERRUPT_INTERVAL, ZINITIX_SCAN_RATE_HZ*ZINITIX_RAW_DATA_ESD_TIMER_INTERVAL)!=I2C_SUCCESS)
			printk("[TSP] [zinitix_touch] Fail to set ZINITIX_RAW_DATA_ESD_TIMER_INTERVAL.\r\n");
	}
	else
#endif
{
...
fail_init:
	...
	printk("[TSP] [zinitix touch] failed to initiallize\r\n");
	return false;
...
static ssize_t zinitix_update_fw(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[TSP] zinitix update fw : name = %s, size = %d\r\n", buf, count);
...
static ssize_t check_binary_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(check_data == 2) {
		printk("[TSP] TSP firmware is not detected\n");
		check_binary = 2;
	}else if(check_data == 1) {
		printk("[TSP] TSP firmware is detected\n");
		check_binary = 1;
	}else {
		printk("[TSP] TSP firmware check binary error\n");
		printk("[TSP] TSP check binary buf data is %s\n", buf);
	}
...
err_detect_failed:
	if (get_hw_revision() > 0x07) {
		printk("[TSP] TSP is not detected. Turn off external LDO\n");
		gpio_direction_output(OMAP_GPIO_AMP_SHDN, 0);
		gpio_free(OMAP_GPIO_AMP_SHDN);
	}
