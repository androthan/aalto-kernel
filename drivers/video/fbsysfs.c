...
#ifdef CONFIG_SAMSUNG_AALTO_CABC
extern int samsung_aalto_set_cabc(const char* level);
static ssize_t store_samsung_aalto_cabc(struct device *device,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{
	if (buf == NULL)
...
static struct device_attribute device_attrs[] = {
	...
	__ATTR(blank, 00664, show_blank, store_blank),	/* Aalto to control blanking from platform. */
...
#endif
#ifdef CONFIG_SAMSUNG_AALTO_CABC
	__ATTR(samsung_aalto_cabc, S_IRUGO|S_IWUSR|S_IWGRP , NULL, store_samsung_aalto_cabc),
#endif
};
