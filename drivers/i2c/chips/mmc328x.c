...
static int mmc328x_init_probe(struct platform_device *pdev)
{
	...
	if (get_hw_revision() > 0x08) {
		printk("[SENSOR] hw_revision : %d\n", get_hw_revision());
...
static int mmc328x_magnetic_resume(struct early_suspend *handler)
{
	...
	if (!regulator_is_enabled(vreg_sensor))
	{
		printk("[SENSOR] Resume mode vreg enable \n");
		ret = regulator_enable(vreg_sensor);
...
#endif
	twl_i2c_read_u8(TWL4030_MODULE_PM_RECEIVER, &temp, TWL4030_VMMC2_DEDICATED);
	printk("mmc328x dedicated is %x\n", temp);
	temp = (temp & 0xF0) | 0x09;
	printk("mmc328x set dedicated is %x\n", temp);
	err = twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, temp, TWL4030_VMMC2_DEDICATED);
	msleep(3);
	if(err)
	{
		printk("mmc328x failed to write TWL4030_VMMC2_DEDICATED!.\n");
	}
	else
	{
		twl_i2c_read_u8(TWL4030_MODULE_PM_RECEIVER, &temp, TWL4030_VMMC2_DEV_GRP);
		printk("mmc328x dev_grp is %x\n", temp);
		temp = (temp & 0x1F) | 0xE0;
		printk("mmc328x set dev_grp is %x\n", temp);
		err = twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, temp, TWL4030_VMMC2_DEV_GRP);
		msleep(3);
		if(err)
			printk("mmc328x failed to write TWL4030_VMMC2_DEV_GRP register.\n");
	}
