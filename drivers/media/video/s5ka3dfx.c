...
static struct s5ka3dfx_sensor s5ka3dfx = {
  ...
  .fps            = 15,
 //.fps            = 30,//LYG
  .preview_size   = S5KA3DFX_PREVIEW_SIZE_640_480, //LYG
  //.preview_size   = S5KA3DFX_PREVIEW_SIZE_320_240,
							...
							int index)
{
	...
	//dev_dbg(&client->dev, "%s: count %d \n", __func__, count);
	 printk(S5KA3DFX_MOD_NAME "SR030pc30_i2c_write start  count = %d, sizeof = %d \n",count, sizeof(unsigned short));
	for (i=0 ; i <count ; i++) {
	...
	printk(S5KA3DFX_MOD_NAME "SR030pc30_i2c_write sucess err = %d  \n",err);
	return 0;
}
...
static int SR030pc30_regs_write(struct i2c_client *client, unsigned short i2c_data[], unsigned short length, char *name)
{
	...
	printk(KERN_DEBUG "%s start, Status is %s mode, parameter name = %s, length =  %d\n",\
	 ...
	 if(CamTunningStatus) // binary mode
 	{
		...
		//if(err==0)
			//printk(S5KA3DFX_MOD_NAME "SR030pc30_regs_write sucess err = %d  \n",err);
...
static int s5ka3dfx_set_effect(s32 value)
{
 ...
 int err=-EINVAL;
  ...
  switch(value)
  {
	...
	case S5KA3DFX_EFFECT_GREY:		//Gray
...
streamon_fail:
  printk(S5KA3DFX_MOD_NAME "ioctl_streamon is failed!!!\n");
  return -EINVAL;   
}
  ...
  switch (vc->id) 
  {
    ...
    case V4L2_CID_CAMERA_CHECK_DATALINE:
	{
	  sensor->check_dataline = vc->value;
	   dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "V4L2_CID_CAMERA_CHECK_DATALINE : sensor->check_dataline=%d\n", sensor->check_dataline);
	  retval = 0;
	}
	  break;	
...
static int ioctl_s_power(struct v4l2_int_device *s, enum v4l2_power on)
{
  ...
  dprintk(CAM_INF, S5KA3DFX_MOD_NAME "ioctl_s_power is called......I2Caddr=0x%x, name = %s\n", client->addr,client->name);

  if(power_Enable ==0||power_Enable ==1){
  	if (gpio_request(OMAP3430_GPIO_CAMERA_EN4,"CAM EN4") != 0) 
  	{
    		printk(S5KA3DFX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_EN4);
   		// return -EIO;
  	}
  ...
  switch(on)
  {
	...
	front_cam_in_use= 1 ;
      ...
      if(err == 0){
		printk(S5KA3DFX_MOD_NAME "SR030pc30_i2c_read n success I2C is OK " S5KA3DFX_DRIVER_NAME " sensor\n");
	  }
      else{
	 front_cam_in_use= 0 ;
        printk(S5KA3DFX_MOD_NAME "Unable to detect-i2c not connect  " S5KA3DFX_DRIVER_NAME " sensor\n");
        sensor->pdata->power_set(V4L2_POWER_OFF);
        return err;
      }
#if 0
	printk(S5KA3DFX_MOD_NAME "SR030pc30 Device ID 0x8c= 0x%x \n!!", read_value); 
	mdelay(3);
	...
	printk(S5KA3DFX_MOD_NAME "SR030pc30 Device ID 0xb1 = 0x%x \n!!", read_value); 
	mdelay(3);	
...
#if 0	
 	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO103 ===%d\n",gpio_get_value(103));
	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO104 ===%d\n",gpio_get_value(104));
	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO105 ===%d\n",gpio_get_value(105));
	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO106 ===%d\n",gpio_get_value(106));
	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO107 ===%d\n",gpio_get_value(107));
	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO108 ===%d\n",gpio_get_value(108));
	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO109 ===%d\n",gpio_get_value(109));
	dprintk(CAM_DBG, S5KA3DFX_MOD_NAME "GPIO110 ===%d\n",gpio_get_value(110));
#endif	
...
static int ioctl_init(struct v4l2_int_device *s)
{
  ...
  //sensor->preview_size              = S5KA3DFX_PREVIEW_SIZE_320_240;
