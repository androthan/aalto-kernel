...
#define dprintk(x, y...)
#endif
#define S5KA3DFX_BIGGEST_FRAME_BYTE_SIZE  PAGE_ALIGN(1280 * 720 * 2 *6) //fix for usage of 6 buffers for 720p capture and avoiding camera launch issues.
...
static int s5ka3dfx_enable_gpio(void)
{
  ...
  {
    printk(S5KA3DFX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_EN3);
   // return -EIO;
  }
  ...
  {
    printk(S5KA3DFX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_STBY);
   // return -EIO;
  }

  if (gpio_request(OMAP3430_GPIO_CAMERA_RST,"CAM VGA RST") != 0) 
  {
    printk(S5KA3DFX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_RST);
   // return -EIO;
  } 
  ...
  {
    printk(S5KA3DFX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_SDA);
    //return -EIO;
  }

  if (gpio_request(OMAP3430_GPIO_CAMERA_SCL,"CAM VGA RST") != 0) 
  {
    printk(S5KA3DFX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_SCL);
   // return -EIO;
  }  
 #endif 
...
//  hwc->u.sensor.xclk = s5ka3dfx_hwc.xclk;
 ...
 hwc->u.sensor.capture_mem = s5ka3dfx_hwc.capture_mem;
  hwc->dev_index = 1;  // 1;
  hwc->dev_minor = 5;   //5;
